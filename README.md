...
- [x] gitでバージョン管理。
- [x] 自身が必要と思う範囲でテストコードを追加してほしい。
- [x] 要件をシンプルに実装してほしい。「今後こういった拡張があるかも」という想定は不要です。
...
- [x] 「写真アップロード画面」へ遷移するリンクがある。
- [x] 「ログアウト」機能を呼び出すリンクがある。
- [x] 未ログイン状態でこの画面にアクセスした場合、ログイン画面を表示する。

## 3. 写真アップロード画面
- [x] 「タイトル」「画像ファイル」を指定して、画像をアップロードできる。
- 以下の場合、エラーメッセージと共にアップロード画面を再表示する。
  - [x] 「タイトル」「画像ファイル」が未入力
  - [x] 「タイトル」の文字数が30文字を超えている
- [x] 画像アップロード処理が成功した場合、2の「写真一覧画面」を表示する。（アップロードした写真が先頭に表示される）
- [x] アップロードをキャンセルし、「写真一覧画面」に戻るリンクがある。
- [x] 4で作成する「ログアウト」機能を呼び出すリンクがある。
- [x] 未ログイン状態でこの画面にアクセスした場合、ログイン画面を表示する。
